...
export interface UserDetailProfile {
  ...
  membership_papers?: {
    id_card?: string | null; // 신분증
    photo?: string | null; // 사진
    family_certificate?: string | null; // 가족관계증명서
    graduation_certificate?: string | null; // 졸업증명서
    employment_certificate?: string | null; // 재직증명서
    license_copy?: string | null; // 자격증 사본
  } | null;
  membership_fee?: string | null; // 가입비
  membership_fee_success?: string | null; // 성혼비
  another_phone?: string[] | null; // 다른 연락처
  ...
  job?: {
    ...
    hire_date: string | null; // 입사 년월
    salary: {
      pre_tax: string; // 세전 연봉
      after_tax: string; // 세후 연봉
      other_income: string; // 그 외 수입
    };
    years_of_service: number; // 재직 연차
  } | null;
  education?: {
    college: EducationPeriod | null;
    phd_school: EducationPeriod | null;
    university: EducationPeriod | null;
    high_school: EducationPeriod | null;
    graduate_school: EducationPeriod | null;
  ...
  family_info?: Array<{
    ...
    death_year?: string | null; // 사망 연도 (생존 시 null)
